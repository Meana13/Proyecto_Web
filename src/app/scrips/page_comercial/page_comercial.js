...
//----------------------------------
/*
funcion cambiarOrden()

//----------------------------------
const columnaFecha = getElementById("fecha-pendientes");
columnaFecha.addEventListener(click, () => {
    function cambiarOrden() {
        ...
        for (var i = 1; i < filas.length; i++) {
            var rowData = filas[i].cells;
            var nombre = rowData[0].innerHTML;
            var apellido = rowData[1].innerHTML;
            var fecha = rowData[2].innerHTML;
        }
        ...
        if (contenedorPendientes.getAttribute("data-orden") === "ascendente") {
            data.sort(function (a, b) {
                return new Date(b.fecha) - new Date(a.fecha);
            });
            contenedorPendientes.setAttribute("data-orden", "descendente");
        } else {
            data.sort(function (a, b) {
                return new Date(a.fecha) - new Date(b.fecha);
            });
            contenedorPendientes.setAttribute("data-orden", "ascendente");
        }

        for (var i = 0; i < data.length; i++) {
            filas[i + 1].cells[0].innerHTML = data[i].nombre;
            filas[i + 1].cells[1].innerHTML = data[i].apellido;
            filas[i + 1].cells[2].innerHTML = data[i].fecha;
        }
    }
})
...
function crearFilaSolicitudPendiente(solicitud) {
    ...
    celdaFecha.addEventListener("click", cambiarOrden);
    ...
    fila.append(celdaNombre, celdaApellidos, celdaFecha, celdaEmail, celdaBoton);
  
    // Resto del código de la función
  
    function cambiarOrden() {
      let table = fila.closest("table"); // Obtener la tabla más cercana al elemento fila
      let rows = Array.from(table.getElementsByTagName("tr")).slice(1); // Obtener todas las filas, excluyendo la primera (encabezados)
  
      if (table.getAttribute("data-orden") === "ascendente") {
        rows.sort(function(a, b) {
          return new Date(b.cells[2].textContent) - new Date(a.cells[2].textContent);
        });
        table.setAttribute("data-orden", "descendente");
      } else {
        rows.sort(function(a, b) {
          return new Date(a.cells[2].textContent) - new Date(b.cells[2].textContent);
        });
        table.setAttribute("data-orden", "ascendente");
      }
  
      for (let i = 0; i < rows.length; i++) {
        table.appendChild(rows[i]); // Agregar las filas ordenadas nuevamente a la tabla
      }
    }
...
function crearFilaSolicitudProceso(solicitud) {
    ...
    celdaEmail.textContent = solicitud.fecha;
